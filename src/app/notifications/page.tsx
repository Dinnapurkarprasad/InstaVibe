...
function NotificationPage() {
  ...
  const [notifiactions,setNotifications]=useState<Notification[]>([])
    ...
    const fetchNotifiactions=async()=>{
      setIsLoading(true)
        try {
          ...
          toast.error("Failed to fetch the Notification")
        }
        finally{
          setIsLoading(false)
        }
        ...
        <CardHeader className="border-b">
          <div className="flex items-center justify-between">
            ...
            <span className="text-sm text-muted-foreground">
              {notifiactions.filter((n) => !n.read).length} unread
            </span>
          </div>
        </CardHeader>
              ...
              notifiactions.map((notification) => (
                ...
                >
                  <Avatar className="mt-1">
                    <AvatarImage src={notification.creator.image ?? "/avatar.png"} className="size-8 rounded-full" />
                  </Avatar>
